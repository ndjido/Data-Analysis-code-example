...
df_validations.drop("AGG_VALIDATIONS_DIVERS_VAL_ARRET_CODE", 1, inplace=True)

df_validations_agg = df_validations.groupby(['AGG_VALIDATIONS_DIVERS_DATE_VALIDATION', 'AGG_VALIDATIONS_DIVERS_LIG_CODE']).sum().reset_index()

df_validations_agg.rename(columns={'AGG_VALIDATIONS_DIVERS_DATE_VALIDATION':'date', 'AGG_VALIDATIONS_DIVERS_LIG_CODE':'ligne'}, inplace=True)

df_validations_agg['annee'] = df_validations_agg.date.map(lambda s: int(s.split('/')[2]))

df_validations_agg = df_validations_agg[df_validations_agg.annee==2014]
...
df_comptage_agg = df_comptage.groupby(["date", "ligne"]).sum().reset_index()

#df_validations_agg['merge_id'] = df_validations_agg.date.map(str) + '_' + df_validations_agg.ligne.map(str)

#df_comptage['merge_id'] = df_comptage.date.map(str) + '_' + df_comptage.ligne.map(str)

df_validation_cpt = pd.merge(df_validations_agg, df_comptage_agg, how="left", on=["date", "ligne"], sort=False)

df_validation_cpt.rename(columns={'AGG_VALIDATIONS_DIVERS_NB_VALIDATIONS':'nb_validations'}, inplace=True)

df_validation_cpt.drop("annee", 1, inplace=True)

df_validation_cpt.drop("NBR_VOY_BRT", 1, inplace=True)

df_validation_cpt['taux_de_non_validation'] = (df_validation_cpt.NBR_VOY_COR - df_validation_cpt.nb_validations)/df_validation_cpt.NBR_VOY_COR

df_validation_cpt.rename(columns={"NBR_VOY_COR":'comptages_corriges'}, inplace=True)

#df_validation_cpt['taux_de_non_valitation_brt'] = (df_validation_cpt.NBR_VOY_BRT - df_validation_cpt.nb_validations)/df_validation_cpt.NBR_VOY_BRT
...
df_validations_cpt_ctr = pd.merge(df_validation_cpt, df_controles_agg, how='left', on=["date", "ligne"], sort=False)
...
df_validations_cpt_ctr_pb = pd.merge(df_validations_cpt_ctr, df_pb_agg, how='left', on=["date", "ligne"], sort=False)
...
df_periodes = df_periodes[['DATE', 'LIBELLE_MOIS',  'LIBELLE_JOUR']]
...
df_validations_cpt_ctr_pb.drop('dateTIME', 1, inplace=True)
...
df_passage = pd.read_csv('../agg_passage_point.csv', sep=';')
...
df_validations_cpt_ctr_pb_meteo_pv_pass.to_csv('Lyon_validations_cpt_ctr_pb_meteo_pv_pass.csv', sep=';', index=False) 
'''

'''
--- df_validations_agg_cible_prod
'''
df_validations_agg_cible_prod = pd.read_csv('df_validations_agg_cible_prod_2.csv', sep=";")

df_validations_agg_cible_prod.drop("arret", 1, inplace=True)

df_validations_agg_cible_prod_agg = df_validations_agg_cible_prod.groupby(["date", "ligne"]).sum().reset_index()

df_validations_cpt_ctr_pb_meteo_pv__cible_prod = pd.merge(df_validations_cpt_ctr_pb_meteo_pv, df_validations_agg_cible_prod_agg, how='left')

df_validations_cpt_ctr_pb_meteo_pv__cible_prod.to_csv('Lyon_validations_cpt_ctr_pb_meteo_pv__cible_prod.csv', sep=';', index=False)


'''
--- df_validations_agg_famille_marketing_prod
'''
df_validations_agg_famille_marketing_prod = pd.read_csv('df_validations_agg_famille_marketing_prod.csv', sep=";")

df_validations_agg_famille_marketing_prod.drop("arret", 1, inplace=True)

df_validations_agg_famille_marketing_prod_agg = df_validations_agg_famille_marketing_prod.groupby(["date", "ligne"]).sum().reset_index()

df_validations_cpt_ctr_pb_meteo_pv__famille_marketing_prod = pd.merge(df_validations_cpt_ctr_pb_meteo_pv, df_validations_agg_famille_marketing_prod_agg, how='left')

df_validations_cpt_ctr_pb_meteo_pv__famille_marketing_prod.to_csv('Lyon_validations_cpt_ctr_pb_meteo_pv__famille_marketing_prod.csv', sep=';', index=False)


'''
--- df_validations_agg_famille_prod
'''
df_validations_agg_famille_prod = pd.read_csv('df_validations_agg_famille_prod.csv', sep=";")

df_validations_agg_famille_prod.drop("arret", 1, inplace=True)

df_validations_agg_famille_prod_agg = df_validations_agg_famille_prod.groupby(["date", "ligne"]).sum().reset_index()

df_validations_cpt_ctr_pb_meteo_pv__famille_prod = pd.merge(df_validations_cpt_ctr_pb_meteo_pv, df_validations_agg_famille_prod_agg, how='left')

df_validations_cpt_ctr_pb_meteo_pv__famille_prod.to_csv('Lyon_validations_cpt_ctr_pb_meteo_pv__famille_prod.csv', sep=';', index=False)


'''
--- df_validations_agg_sous_famille_prod
'''
df_validations_agg_sous_famille_prod = pd.read_csv('df_validations_agg_sous_famille_prod.csv', sep=";")

df_validations_agg_sous_famille_prod.drop("arret", 1, inplace=True)

df_validations_agg_sous_famille_prod_agg = df_validations_agg_sous_famille_prod.groupby(["date", "ligne"]).sum().reset_index()

df_validations_cpt_ctr_pb_meteo_pv__sous_famille_prod = pd.merge(df_validations_cpt_ctr_pb_meteo_pv, df_validations_agg_sous_famille_prod_agg, how='left')

df_validations_cpt_ctr_pb_meteo_pv__sous_famille_prod.to_csv('Lyon_validations_cpt_ctr_pb_meteo_pv__sous_famille_prod.csv', sep=';', index=False)


'''
--- df_validations_agg_sous_type_prod
'''
df_validations_agg_sous_type_prod = pd.read_csv('df_validations_agg_sous_type_prod.csv', sep=";")

df_validations_agg_sous_type_prod.drop("arret", 1, inplace=True)

df_validations_agg_sous_type_prod_agg = df_validations_agg_sous_type_prod.groupby(["date", "ligne"]).sum().reset_index()

df_validations_cpt_ctr_pb_meteo_pv__sous_type_prod = pd.merge(df_validations_cpt_ctr_pb_meteo_pv, df_validations_agg_sous_type_prod_agg, how='left')

df_validations_cpt_ctr_pb_meteo_pv__sous_type_prod.to_csv('Lyon_validations_cpt_ctr_pb_meteo_pv__sous_type_prod.csv', sep=';', index=False)
